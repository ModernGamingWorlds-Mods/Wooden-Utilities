...
public class WoodenAnvilMenu extends ItemCombinerMenu {
    ...
    public void createResult() {
        ...
        if (!hammer.isEmpty() && hammer.is(ModItems.WOODEN_HAMMER.get()) && !plank.isEmpty()) {
            Level level = this.access.evaluate((l, p) -> l, null);
            if (level != null) {
                RecipeManager rm = level.getRecipeManager();
                //? if recipe_holder {
                /*for (net.minecraft.world.item.crafting.RecipeHolder<WoodenAnvilRecipe> holder :
                        rm.getAllRecipesFor(ModRecipes.WOODEN_ANVIL_TYPE.get())) {
                    WoodenAnvilRecipe recipe = holder.value();
                *///?} else {
                for (WoodenAnvilRecipe recipe : rm.getAllRecipesFor(ModRecipes.WOODEN_ANVIL_TYPE.get())) {
                //?}
                    if (recipe.getInput().test(plank)) {
                        this.resultSlots.setItem(0, recipe.getResult().copy());
